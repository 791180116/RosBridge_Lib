...
public class Topic<T extends Message> extends LinkedBlockingQueue<T> implements FullMessageHandler {
    ...
    /**
     * warning: there is a delay between the completion of this method and the completion of the subscription;
     * it takes longer than publishing multiple other messages, for example.
     * 警告：此方法完成与订阅完成之间存在延迟；例如，它比发布多个其他消息需要更长的时间。
     */
    ...
    public void unsubscribe() {
        /**
         * need to handle race conditions in incoming message handler so that once unsubscribe has happened the handler gets no more messages
         * 需要处理传入消息处理程序中的竞争条件，以便一旦取消订阅，处理程序就不会收到更多消息
         */
